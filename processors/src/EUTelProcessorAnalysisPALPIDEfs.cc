...
// - First, we create a rectangle, which is bigger than the cluster in every direction.
// - Second, we fill the hitPixelVec with pixels in this rectangle, which did not fired.
// - Third, we run the clustering process with this hitPixelVec.
// - Fourth, when this process finds one whole cluster and hitPixelVec is still not empty, the original cluster has to have an empty middle, since at least two cluster could be made from the empty part (one on the outside and at least one in the middle of the cluster).
